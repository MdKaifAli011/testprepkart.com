...
async function importNeetComments() {
  const uri = process.env.DATABASE_URI || 'mongodb://127.0.0.1:27017/Demo_testprepkart_backend'
  const client = new MongoClient(uri)

  try {
    console.log('💬 Importing NEET Comments with Blog Relationships')
    ...
    // Step 1: Read the NEET comments JSON file
    console.log('\n📄 Step 1: Reading NEET comments data...')
    console.log('-'.repeat(40))

    const commentsFilePath = path.join(process.cwd(), '..', 'sql', 'neet-comments.json')
    ...
    // Step 2: Get all NEET blogs to create mapping
    console.log('\n🔍 Step 2: Creating blog ID mapping...')
    console.log('-'.repeat(40))

    const neetBlogs = await db.collection('neetblogs').find({}).toArray()
    console.log(`📚 Found ${neetBlogs.length} NEET blogs in database`)

    // Create mapping from originalId to MongoDB _id
    const blogMapping = {}
    neetBlogs.forEach((blog) => {
      if (blog.originalId) {
    ...
    for (const comment of comments) {
      try {
        ...
        const existingComment = await db.collection('neet_comments').findOne({
        ...
        // Insert comment
        await db.collection('neet_comments').insertOne(commentDoc)
        commentsInserted++
    ...
    const finalStats = {
      totalComments: await db.collection('neet_comments').countDocuments(),
      approvedComments: await db.collection('neet_comments').countDocuments({ status: 'approved' }),
      spamComments: await db.collection('neet_comments').countDocuments({ status: 'spam' }),
    ...
    const importSummary = {
      importInfo: {
        ...
        sourceFile: 'neet-comments.json',
        targetCollection: 'neet_comments',
    ...
    const exportFile = path.join(exportDir, `neet-comments-import-summary-${timestamp}.json`)

    fs.writeFileSync(exportFile, JSON.stringify(importSummary, null, 2))
    console.log(`✅ Import summary saved to: ${exportFile}`)

    // Final Summary
    console.log('\n🎉 NEET COMMENTS IMPORT COMPLETED!')
    console.log('='.repeat(60))
    console.log(`📊 Summary:`)
    ...
    console.log('\n🚀 Next Steps:')
    console.log('1. Review comments in Payload CMS admin panel at /admin/collections/neet_comments')
    ...
    console.error('❌ Error importing NEET comments:', error)
    throw error
  } finally {
    await client.close()
  }
...
module.exports = { importNeetComments }
