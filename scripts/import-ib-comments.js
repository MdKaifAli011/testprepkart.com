...
async function importIbComments() {
  ...
  try {
    ...
    // Step 1: Read the IB comments JSON file
    console.log('\n📄 Step 1: Reading IB comments data...')
    console.log('-'.repeat(40))

    const commentsFilePath = path.join(process.cwd(), '..', 'sql', 'ib-comments.json')
    ...
    // Step 2: Get all IB blogs to create mapping
    console.log('\n🔍 Step 2: Creating blog ID mapping...')
    console.log('-'.repeat(40))

    const ibBlogs = await db.collection('idblogs').find({}).toArray()
    console.log(`📚 Found ${ibBlogs.length} IB blogs in database`)

    // Create mapping from originalId to MongoDB _id
    const blogMapping = {}
    ibBlogs.forEach((blog) => {
      if (blog.originalId) {
    ...
    for (const comment of comments) {
      try {
        ...
        const existingComment = await db.collection('ib_comments').findOne({
        ...
        // Insert comment
        await db.collection('ib_comments').insertOne(commentDoc)
        commentsInserted++
    ...
    const finalStats = {
      totalComments: await db.collection('ib_comments').countDocuments(),
      approvedComments: await db.collection('ib_comments').countDocuments({ status: 'approved' }),
      spamComments: await db.collection('ib_comments').countDocuments({ status: 'spam' }),
    ...
    const importSummary = {
      importInfo: {
        ...
        sourceFile: 'ib-comments.json',
        targetCollection: 'ib_comments',
    ...
    console.log('\n🚀 Next Steps:')
    console.log('1. Review comments in Payload CMS admin panel at /admin/collections/ib_comments')
    ...
    console.error('❌ Error importing IB comments:', error)
    throw error
  } finally {
    await client.close()
  }
...
function isSpamComment(comment) {
  ...
  const hasSuspiciousEmail =
    ...
    (email.includes('spam') ||
      ...
      email.includes('fake') ||
      email.includes('test') ||
      email.includes('mozmail') ||
      email.includes('directinboxs'))
  ...
  // Check for Russian/Cyrillic spam patterns
  const hasCyrillicSpam =
    /[а-яё]/i.test(content) &&
    (content.includes('видео') || content.includes('наблюдение') || content.includes('регистратор'))
