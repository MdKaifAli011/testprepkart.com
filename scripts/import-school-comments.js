...
async function importSchoolComments() {
  const uri = process.env.DATABASE_URI || 'mongodb://127.0.0.1:27017/Demo_testprepkart_backend'
  const client = new MongoClient(uri)

  try {
    console.log('💬 Importing School Exam Comments with Blog Relationships')
    ...
    // Step 1: Read the school comments JSON file
    console.log('\n📄 Step 1: Reading school comments data...')
    console.log('-'.repeat(40))

    const commentsFilePath = path.join(process.cwd(), '..', 'sql', 'school exam-comment.json')
    ...
    // Step 2: Get all school blogs to create mapping
    console.log('\n🔍 Step 2: Creating blog ID mapping...')
    console.log('-'.repeat(40))

    const schoolBlogs = await db.collection('schoolblogs').find({}).toArray()
    console.log(`📚 Found ${schoolBlogs.length} school blogs in database`)

    // Create mapping from originalId to MongoDB _id
    const blogMapping = {}
    schoolBlogs.forEach((blog) => {
      if (blog.originalId) {
    ...
    for (const comment of comments) {
      try {
        ...
        const existingComment = await db.collection('school_comments').findOne({
        ...
        // Insert comment
        await db.collection('school_comments').insertOne(commentDoc)
        commentsInserted++

        if (commentsInserted % 50 === 0) {
    ...
    const finalStats = {
      totalComments: await db.collection('school_comments').countDocuments(),
      approvedComments: await db
        .collection('school_comments')
        .countDocuments({ status: 'approved' }),
      spamComments: await db.collection('school_comments').countDocuments({ status: 'spam' }),
    ...
    const importSummary = {
      importInfo: {
        ...
        sourceFile: 'school exam-comment.json',
        targetCollection: 'school_comments',
    ...
    const exportFile = path.join(exportDir, `school-comments-import-summary-${timestamp}.json`)

    fs.writeFileSync(exportFile, JSON.stringify(importSummary, null, 2))
    console.log(`✅ Import summary saved to: ${exportFile}`)

    // Final Summary
    console.log('\n🎉 SCHOOL EXAM COMMENTS IMPORT COMPLETED!')
    console.log('='.repeat(60))
    console.log(`📊 Summary:`)
    ...
    console.log('\n🚀 Next Steps:')
    console.log(
      '1. Review comments in Payload CMS admin panel at /admin/collections/school_comments',
    )
    ...
    console.error('❌ Error importing school comments:', error)
    throw error
  } finally {
    await client.close()
  }
...
function isSpamComment(comment) {
  ...
  const spamAuthors = ['spam', 'bot', 'admin', 'test', 'fake', 'dummy', 'cpjjwwhv']
...
module.exports = { importSchoolComments }
