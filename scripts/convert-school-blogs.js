...
async function convertSchoolBlogs() {
  const uri = process.env.DATABASE_URI || 'mongodb://127.0.0.1:27017/Demo_testprepkart_backend'
  const client = new MongoClient(uri)

  const TARGET_EXAM_ID = '68da71472deacee8b09f05c4' // School Exam

  try {
    console.log('🔄 Converting School SQL to School Blogs Collection')
    ...
    // Step 1: Get the School Exam ID
    console.log('\n🔍 Step 1: Finding School Exam...')
    console.log('-'.repeat(40))

    const schoolExam = await db.collection('exams').findOne({
      _id: new ObjectId(TARGET_EXAM_ID),
    })

    if (!schoolExam) {
      console.log('❌ School Exam not found!')
      return
    }

    console.log(`✅ Found School Exam: ${schoolExam.examName}`)
    console.log(`   - ID: ${schoolExam._id}`)
    console.log(`   - Original ID: ${schoolExam.originalId}`)

    // Step 2: Read the SQL JSON file
    console.log('\n📄 Step 2: Reading School SQL data...')
    console.log('-'.repeat(40))

    const sqlFilePath = path.join(process.cwd(), '..', 'sql', 'School_Exam.json')
    ...
    console.log(`📊 Found ${dataLines.length} blog posts in School SQL data`)
    ...
    console.log('\n📚 Step 3: Preparing to insert into schoolblogs collection...')
    console.log('-'.repeat(40))

    console.log(`📊 Target Collection: schoolblogs`)
    console.log(`   - Mapping all blogs to exam: ${schoolExam.examName}`)
    console.log(`   - Exam ID: ${schoolExam._id}`)
    ...
    for (const post of postsData) {
      try {
        ...
        // Check if blog already exists in schoolblogs collection
        const existingBlog = await db.collection('schoolblogs').findOne({
        ...
        let title = `School Blog ${originalId}`
        let excerpt = ''
        ...
        const keywordsArray = extractSchoolKeywordsFromContent(post.description)
        const keywordsString = keywordsArray.join(', ')

        // Create blog document matching SchoolBlogs schema
        const blogDoc = {
          originalId: originalId,
          title: title,
          ...
          exam: schoolExam._id, // Required relationship
          status: 'published',
        ...
        // Insert blog into schoolblogs collection
        await db.collection('schoolblogs').insertOne(blogDoc)
        blogsInserted++
    ...
    const blogsWithEmptyTitles = await db
      ...
      .find({
        ...
        exam: schoolExam._id, // Only fix school blogs
      })
      .toArray()
    ...
    for (const blog of blogsWithEmptyTitles) {
      try {
        const newTitle = `School Blog ${blog.originalId}`
        await db.collection('schoolblogs').updateOne(
          { _id: blog._id },
          {
            $set: {
              title: newTitle,
    ...
    for (const blog of blogsNeedingDefaults) {
      try {
        ...
        if (Object.keys(updates).length > 0) {
          ...
          await db.collection('schoolblogs').updateOne({ _id: blog._id }, { $set: updates })
    ...
    const totalBlogs = await db.collection('schoolblogs').countDocuments({
      exam: schoolExam._id,
    })
    ...
    console.log(`  📝 Total School blogs: ${totalBlogs}`)
    ...
    console.log('📋 Sample School blogs with exam relationships:')
    ...
    const blogsWithEmptyTitle = await db.collection('schoolblogs').countDocuments({
      $or: [{ title: { $in: ['', null] } }, { title: { $exists: false } }],
      exam: schoolExam._id,
    })
    const blogsWithEmptyExcerpt = await db.collection('schoolblogs').countDocuments({
      $or: [{ excerpt: { $in: ['', null] } }, { excerpt: { $exists: false } }],
      exam: schoolExam._id,
    })
    ...
    const exportSummary = {
      conversionInfo: {
        ...
        sourceFile: 'School_Exam.json',
        targetCollection: 'schoolblogs',
        examId: schoolExam._id,
        examName: schoolExam.examName,
      ...
      statistics: {
        ...
        fixedDefaults,
        totalBlogs,
        examMapped: schoolExam.examName,
      },
      dataQuality: {
        blogsWithEmptyTitle,
    ...
    const exportFile = path.join(exportDir, `school-blog-conversion-summary-${timestamp}.json`)

    fs.writeFileSync(exportFile, JSON.stringify(exportSummary, null, 2))
    console.log(`✅ Export summary saved to: ${exportFile}`)

    // Final Summary
    console.log('\n🎉 SCHOOL BLOGS CONVERSION & FIX COMPLETED!')
    console.log('='.repeat(60))
    console.log(`📊 Final Results:`)
    console.log(`  🎯 Target Collection: schoolblogs`)
    console.log(`  🎯 Target Exam: ${schoolExam.examName}`)
    console.log(`  📝 Total School Blogs: ${totalBlogs}`)
    ...
    console.log('\n🚀 Next Steps:')
    console.log(
      '1. Verify School blogs in Payload CMS admin panel at /admin/collections/schoolblogs',
    )
    console.log('2. Create API endpoints for School blog listing')
    console.log('3. Build School blog display pages')
...
// Helper function to extract school keywords from content (returns array)
function extractSchoolKeywordsFromContent(content) {
  if (!content) return []
  ...
  if (contentLower.includes('school')) keywords.push('School')
  if (contentLower.includes('education')) keywords.push('Education')
  if (contentLower.includes('student')) keywords.push('Student')
  if (contentLower.includes('teacher')) keywords.push('Teacher')
  if (contentLower.includes('class')) keywords.push('Class')
  if (contentLower.includes('grade')) keywords.push('Grade')
  if (contentLower.includes('exam')) keywords.push('Exam')
  if (contentLower.includes('test')) keywords.push('Test')
  if (contentLower.includes('study')) keywords.push('Study')
  if (contentLower.includes('learning')) keywords.push('Learning')
  if (contentLower.includes('curriculum')) keywords.push('Curriculum')
