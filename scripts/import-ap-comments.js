...
async function importApComments() {
  ...
  try {
    ...
    // Step 1: Read the AP comments JSON file
    console.log('\n📄 Step 1: Reading AP comments data...')
    console.log('-'.repeat(40))

    const commentsFilePath = path.join(process.cwd(), '..', 'sql', 'ap-comments.json')
    ...
    // Step 2: Get all AP blogs to create mapping
    console.log('\n🔍 Step 2: Creating blog ID mapping...')
    console.log('-'.repeat(40))

    const apBlogs = await db.collection('apblogs').find({}).toArray()
    console.log(`📚 Found ${apBlogs.length} AP blogs in database`)

    // Create mapping from originalId to MongoDB _id
    const blogMapping = {}
    apBlogs.forEach((blog) => {
      if (blog.originalId) {
    ...
    for (const comment of comments) {
      try {
        ...
        const existingComment = await db.collection('ap_comments').findOne({
        ...
        // Insert comment
        await db.collection('ap_comments').insertOne(commentDoc)
        commentsInserted++

        if (commentsInserted % 10 === 0) {
    ...
    const finalStats = {
      totalComments: await db.collection('ap_comments').countDocuments(),
      approvedComments: await db.collection('ap_comments').countDocuments({ status: 'approved' }),
      spamComments: await db.collection('ap_comments').countDocuments({ status: 'spam' }),
    ...
    const importSummary = {
      importInfo: {
        ...
        sourceFile: 'ap-comments.json',
        targetCollection: 'ap_comments',
    ...
    console.log('\n🚀 Next Steps:')
    console.log('1. Review comments in Payload CMS admin panel at /admin/collections/ap_comments')
    ...
    console.error('❌ Error importing AP comments:', error)
    throw error
  } finally {
    await client.close()
  }
...
function isSpamComment(comment) {
  ...
  const spamAuthors = ['spam', 'bot', 'admin', 'test', 'fake', 'dummy', 'cpjjwwhv', 'mjj']
  ...
  const isSuspiciousLength = contentLength < 5 || contentLength > 2000
  ...
  const hasSQLInjection =
    /(union|select|insert|update|delete|drop|create|alter|exec|execute)/i.test(content)

  // Check for XSS patterns
  const hasXSS = /<script|javascript:|on\w+\s*=/i.test(content)

  // Check for specific SQL injection patterns in AP comments
  const hasAPSQLInjection =
    content.includes('sleep(15)') ||
    content.includes('waitfor delay') ||
    content.includes('pg_sleep') ||
    content.includes('dbms_pipe') ||
    content.includes('xor') ||
    content.includes('select(0)from(select(sleep(15)))')
