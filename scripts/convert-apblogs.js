...
async function convertApBlogs() {
  ...
  try {
    console.log('🔄 Converting AP Blogs SQL to AP Blogs Collection')
    ...
    // Step 2: Skip category lookup (not needed for AP blogs)
    console.log('\n📚 Step 2: Skipping category lookup (not needed for AP blogs)...')
    console.log('-'.repeat(40))
    console.log('✅ No categories required - mapping only to exam')

    // Step 3: Read the SQL JSON file
    console.log('\n📄 Step 3: Reading AP Blogs SQL data...')
    console.log('-'.repeat(40))

    const sqlFilePath = path.join(process.cwd(), '..', 'sql', 'apBlogs.json')
    ...
    console.log(`📊 Found ${postsData.length} blog posts in AP Blogs SQL data`)
    console.log(
      `   - SQL Fields: ID, author, createdAt, description, title, excerpt, status, commentStatus, name, updatedAt, commentCount`,
    )
    console.log(
    ...
    console.log('\n📚 Step 4: Preparing to insert into apblogs collection...')
    console.log('-'.repeat(40))

    console.log(`📊 Target Collection: apblogs`)
    ...
    for (const post of postsData) {
      try {
        ...
        // Check if blog already exists in apblogs collection
        const existingBlog = await db.collection('apblogs').findOne({
        ...
        let title = post.title || `AP Blog ${originalId}`
        ...
        const keywordsArray = extractApKeywordsFromContent(post.description)
        const keywordsString = keywordsArray.join(', ')

        // Handle status from SQL data
        const status =
          post.status === 'publish' ? 'published' : post.status === 'inherit' ? 'draft' : 'draft'

        // Handle dates from SQL data
        const publishedAt = post.createdAt ? new Date(post.createdAt) : new Date()
        const createdAt = post.createdAt ? new Date(post.createdAt) : new Date()
        const updatedAt =
          post.updatedAt && post.updatedAt !== '0000-00-00 00:00:00'
            ? new Date(post.updatedAt)
            : new Date()

        // Create blog document matching ApBlogs schema with all SQL fields
        const blogDoc = {
          originalId: originalId,
          title: title,
          slug: post.name && post.name !== '0' ? post.name : `ap-blog-${originalId}`, // Use name as slug or create one
          ...
          author: post.author ? post.author.toString() : '2', // Use author from SQL or default
        ...
        // Insert blog into apblogs collection
        await db.collection('apblogs').insertOne(blogDoc)
        blogsInserted++

        if (blogsInserted % 50 === 0) {
    ...
    for (const blog of blogsWithEmptyTitles) {
      try {
        const newTitle = `AP Blog ${blog.originalId}`
        await db.collection('apblogs').updateOne(
          { _id: blog._id },
          {
            $set: {
              title: newTitle,
    ...
    for (const blog of blogsNeedingDefaults) {
      try {
        ...
        if (Object.keys(updates).length > 0) {
          ...
          await db.collection('apblogs').updateOne({ _id: blog._id }, { $set: updates })
    ...
    const totalBlogs = await db.collection('apblogs').countDocuments({
      exam: internationalExam._id,
    })
    ...
    console.log(`  📝 Total AP blogs: ${totalBlogs}`)
    ...
    console.log('📋 Sample AP blogs with exam relationships:')
    ...
    const blogsWithEmptyTitle = await db.collection('apblogs').countDocuments({
      $or: [{ title: { $in: ['', null] } }, { title: { $exists: false } }],
      exam: internationalExam._id,
    })
    const blogsWithEmptyExcerpt = await db.collection('apblogs').countDocuments({
      $or: [{ excerpt: { $in: ['', null] } }, { excerpt: { $exists: false } }],
      exam: internationalExam._id,
    })
    ...
    const exportSummary = {
      conversionInfo: {
        ...
        sourceFile: 'apBlogs.json',
        targetCollection: 'apblogs',
    ...
    const exportFile = path.join(exportDir, `ap-blog-conversion-summary-${timestamp}.json`)

    fs.writeFileSync(exportFile, JSON.stringify(exportSummary, null, 2))
    console.log(`✅ Export summary saved to: ${exportFile}`)

    // Final Summary
    console.log('\n🎉 AP BLOGS CONVERSION & FIX COMPLETED!')
    console.log('='.repeat(60))
    ...
    console.log('\n🚀 Next Steps:')
    console.log('1. Verify AP blogs in Payload CMS admin panel at /admin/collections/apblogs')
    console.log('2. Create API endpoints for AP blog listing')
    console.log('3. Build AP blog display pages')
...
// Helper function to extract AP-specific keywords from content (returns array)
function extractApKeywordsFromContent(content) {
  if (!content) return []

  const keywords = []
  const contentLower = content.toLowerCase()

  // AP-specific keywords
  if (contentLower.includes('ap')) keywords.push('AP')
  if (contentLower.includes('advanced placement')) keywords.push('Advanced Placement')
  if (contentLower.includes('college board')) keywords.push('College Board')
  if (contentLower.includes('university')) keywords.push('University')
  if (contentLower.includes('college')) keywords.push('College')
  if (contentLower.includes('admission')) keywords.push('Admission')
  if (contentLower.includes('application')) keywords.push('Application')
  if (contentLower.includes('undergraduate')) keywords.push('Undergraduate')
  if (contentLower.includes('graduate')) keywords.push('Graduate')
  ...
  if (contentLower.includes('calculus')) keywords.push('Calculus')
  if (contentLower.includes('statistics')) keywords.push('Statistics')
  ...
  if (contentLower.includes('computer science')) keywords.push('Computer Science')
  if (contentLower.includes('environmental science')) keywords.push('Environmental Science')
