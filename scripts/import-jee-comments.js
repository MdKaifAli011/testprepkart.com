...
async function importJeeComments() {
  const uri = process.env.DATABASE_URI || 'mongodb://127.0.0.1:27017/Demo_testprepkart_backend'
  const client = new MongoClient(uri)

  try {
    console.log('💬 Importing JEE Comments with Blog Relationships')
    ...
    // Step 1: Read the JEE comments JSON file
    console.log('\n📄 Step 1: Reading JEE comments data...')
    console.log('-'.repeat(40))

    const commentsFilePath = path.join(process.cwd(), '..', 'sql', 'jee-comment.json')
    ...
    for (const comment of comments) {
      try {
        ...
        const existingComment = await db.collection('jee_comments').findOne({
        ...
        // Insert comment
        await db.collection('jee_comments').insertOne(commentDoc)
        commentsInserted++
    ...
    const finalStats = {
      totalComments: await db.collection('jee_comments').countDocuments(),
      approvedComments: await db.collection('jee_comments').countDocuments({ status: 'approved' }),
      spamComments: await db.collection('jee_comments').countDocuments({ status: 'spam' }),
    ...
    const importSummary = {
      importInfo: {
        ...
        sourceFile: 'jee-comment.json',
        targetCollection: 'jee_comments',
    ...
    console.log('\n🚀 Next Steps:')
    console.log('1. Review comments in Payload CMS admin panel at /admin/collections/comments')
    ...
    console.error('❌ Error importing JEE comments:', error)
    throw error
  } finally {
    await client.close()
  }
...
module.exports = { importJeeComments }
