...
async function convertInternationalBlogs() {
  ...
  try {
    ...
    console.log('-'.repeat(40))
    console.log('✅ Categories not required for SAT blogs')
    ...
    // Parse the JSON file - it's a proper JSON array
    let postsData = []
    try {
      const jsonData = JSON.parse(fileContent)

      // The structure is: [header, database, table, data_array]
      if (Array.isArray(jsonData) && jsonData.length >= 4) {
        postsData = jsonData[3] // The data is in the 4th element (index 3)
      }
    } catch (error) {
    ...
    console.log(`   - No categories needed (direct exam mapping)`)
    ...
    for (const post of postsData) {
      try {
        ...
        const keywordsArray = extractInternationalKeywordsFromContent(post.description)
        const keywordsString = keywordsArray.join(', ')

        // Create blog document matching SatBlogs schema
        const blogDoc = {
          originalId: originalId,
          title: title,
          ...
          exam: internationalExam._id, // Required relationship
          status: 'published',
          author: post.author, // Keep original author or default to '5'
          metaTitle: title,
          ...
          views: 0,
          publishedAt: new Date(Date.now()), // Use current date since no createAt field
          createdAt: new Date(),
          updatedAt: new Date(),
        }
    ...
    const exportSummary = {
      conversionInfo: {
        ...
        schemaType: 'Simplified (text fields, no richText, no categories)',
      },
      statistics: {
        blogsInserted,
        blogsSkipped,
    ...
    console.log(`  🎯 Target Collection: satblogs`)
    console.log(`  🎯 Target Exam: ${internationalExam.examName}`)
    console.log(`  📝 Total SAT Blogs: ${totalBlogs}`)
    ...
    console.log(`  📚 Schema: Simplified (text fields, no categories)`)
    ...
    console.log('4. Test exam and category relationship filtering')
...
// Helper function to extract international keywords from content (returns array)
function extractInternationalKeywordsFromContent(content) {
  if (!content) return []
