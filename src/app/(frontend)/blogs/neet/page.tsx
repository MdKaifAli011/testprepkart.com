    ...
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-12">
          ...
          <h1 className="text-5xl font-bold mb-4 gradient-text">NEET Preparation Blogs</h1>
          <p className="text-gray-600 text-lg">Expert guidance for medical entrance exams</p>
        </div>
              ...
              <div className="flex justify-center gap-2">
                ...
                {Array.from({ length: blogsResponse.totalPages }, (_, i) => i + 1).map(
                  (pageNum) => (
                    <Link
                      key={pageNum}
                      href={`/blogs/neet?page=${pageNum}`}
                      className={`px-4 py-2 rounded-lg transition-colors ${pageNum === page ? 'bg-purple-600 text-white' : 'bg-white hover:bg-gray-100'}`}
                    >
                      {pageNum}
                    </Link>
                  ),
